...
void search(unsigned int ip){
	...
	if(sTable[x].nexthop != 256) temp = sTable[x].nexthop;
		...
		for(int y=0;y<s;y++){
			count1 += sTable[x].bitmap[y];
		}
		temp = sTable[x].compressedTable[count1];
	}
	if(temp==-1)
	  printf("default\n");
}
...
void create(){
	...
	for(i=0;i<65536;i++){//計算樹高分配第二層表大小
		if(sTable[i].ptr != NULL){
			...
			int secondSize = (int)pow(2,sTable[i].h);
			sTable[i].secTable = (unsigned int *)malloc(sizeof(unsigned int) * secondSize);
			sTable[i].bitmap = (unsigned int *)malloc(sizeof(unsigned int) * secondSize);
		}
	}
	for(i=0; i<num_entry; i++){//建第二層表
		...
		if(table[i].len > 16){
			...
			sTable[firstindex].compressedTable = (int *)malloc(sizeof(int) * compressedSize);
		}
	}
	for(i=0; i<num_entry; i++){ 
		if(table[i].len>16){
			...
			unsigned temp = 257;
			int x = (table[i].ip)>>16;
			int secondSize = (int)pow(2,sTable[x].h);
			for(int u=0;u<secondSize;u++){
				if(sTable[x].secTable[u] != temp){
					sTable[x].compressedTable[countbits] = sTable[x].secTable[u];
					countbits += 1;
					sTable[x].bitmap[u] = 1;
					temp = sTable[x].secTable[u];
				}
				else{
					sTable[x].bitmap[u] = 0;
				}
